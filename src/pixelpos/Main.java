...
public class Main {
	...
	public static void PosHeader(Connection con,int idEmployee,int NumFactura,int MemCode,int MethodNum,double Change,
		...
		try {
			...
			//Insertando nueva transacci贸n en DBA.Howpaid
			ts = new Timestamp(System.currentTimeMillis());
	...
	public void main(ArrayList<capaModelo.DetallePedidoPixel> pruebaPedido, String dsnODBC) {
		 ...
		 try
		 {
			 ...
			 rs.close();
			 System.out.println("D铆a De Apertura: "+DiaApertura);
			 ...
			 */
			 ...
			 //Insertando nueva transacci贸n en dba.PosHeader y en DBA.Howpaid
			 PosHeader(con,2007,NumFactura,MemCode,MethodNum,Change,Inicialts,DiaApertura);
			 
			 //Insertando nueva transacci贸n en dba.Tabinfo
			 ts = new Timestamp(System.currentTimeMillis());
			 ...
			 //Insertando nueva transacci贸n en dba.PosHDelivery
			 ts = new Timestamp(System.currentTimeMillis());
			 ...
			 //Insertando nueva transacci贸n en DBA.POSDETAIL
			 RunPosDetail(con);
